...
void ContentPreviewGrid::updatePreviews() {
    // Before clearing, deselect the currently selected card if it's no longer in the filtered list
    if (m_selectedCardWidget && !m_proxyModel->filterAcceptsRow(m_selectedCardWidget->getContent(), m_proxyModel->index(0,0))) {
        deselectCurrentCard();
    }
    ...
    for (int i = 0; i < count; ++i) {
        ...
        Content *content = m_proxyModel->data(proxyIndex, ContentModel::ContentRole).value<Content*>();
        ...
        if (content) {
            ...
            // Re-select card if it was previously selected
            if (m_selectedCardWidget && m_selectedCardWidget->getContent() == content) {
